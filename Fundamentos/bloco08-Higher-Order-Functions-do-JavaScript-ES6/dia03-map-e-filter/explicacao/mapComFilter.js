const estudantes = [
  ...
  // Função para buscar e imprimir o nome completo de todos os alunos da manhã
  // Com For
  const alllNameStudents = [];

for (let index = 0; index < estudantes.length; index += 1 ) {
  if(estudantes[index].turno === 'Manhã') {
    alllNameStudents.push(`${estudantes[index].nome} ${estudantes[index].sobrenome}`)
  }
}

console.log(alllNameStudents);
